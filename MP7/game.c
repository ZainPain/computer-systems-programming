...
*/
{
    ...
    new_game->score = 0;
    int i;
    unsigned int numcell = rows * cols;
    new_game->cells = calloc( numcell, sizeof(cell));
    for( i = 0; i < numcell; i++)
            new_game->cells[i] = -1;
	return new_game;
}
    ...
    This includes any substructures the game data structure contains. */
{
       ...
       if(cur_game != NULL){
             ...
             free(cur_game);
             cur_game = NULL;
             //setting pointer to null good cs practice
...
*/{
    if(row < 0 || row >= cur_game->rows || col < 0 || col >= cur_game->cols)
            return NULL;
    // if the give row or col are negative or exceed the given amount of rows or
    // columns in the game instance the a NULL is returned
