 ...
 * *** INTRO PARAGRAPH ***
 *
 * This program uses an abstract data structure known as a slicing tree,
 * which is used for floorplanning. The file contains multiple functions
 * for purposes of intializing the slicing tree, as well as traversing and
 * modifying the tree.
 */
...
bool areIdentical(node_t * a, node_t * b)
{
        /* base cases */
        if (a == NULL && b == NULL)
                return true;
        ...
        return (a == b  && areIdentical(a->left, b->left) && areIdentical(a->right, b->right) );
}
...
void swap_module(node_t* a, node_t* b) {
        ...
        //swapperino the mudolerinos
        module_t * temp_lmao = a->module;
        a->module = b->module;
        b->module = temp_lmao;
}
...
void swap_topology(node_t* a, node_t* b) {
        ...
        //parenterinos
        node_t * papa = a->parent;
        node_t * mama = b->parent;
        //the old switcheroo
        if(papa->left == a)
                papa->left = b;
        else
        ...
        if(mama->left == b)
                ...
                mama->right = a;
        //swapperino the parent pointerinos
        a->parent = mama;
        b->parent = papa;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
        ...
        //make that cut node thing
        node_t * node = (node_t *)malloc(sizeof(node_t));
        node->cutline = V;
        node->parent = par;
        node->module = NULL;
       
        //the right child (module)
        node_t * num  = (node_t *)malloc(sizeof(node_t));
        num->cutline = UNDEFINED_CUTLINE;
        num->module = &modules[n];
        num->parent = node;
        num->left = NULL;
        num->right = NULL;
       
        //setting the right and left of the node
        node->right = num;
        //recurse
